...
| `signer_roles`<sup>*_required_</sup> | [```List[SubTemplateRole]```](SubTemplateRole.md) |  An array of the designated signer roles that must be specified when sending a SignatureRequest using this Template.  |  |
...
| `ignore_text_tags_extraction_errors` | ```bool``` |  _t__TemplateCreate::IGNORE_TEXT_TAGS_EXTRACTION_ERRORS  |  [default to False] |
