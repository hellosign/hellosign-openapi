...
class UnclaimedDraftCreateEmbeddedWithTemplateRequest(ModelNormal):
    ...
    def _from_openapi_data(cls, client_id, requester_email_address, template_ids, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            files ([file_type]): Use `files[]` to append additional files to the signature request being created from the template. Dropbox Sign will parse the files for [text tags](https://app.hellosign.com/api/textTagsWalkthrough) and append it to the signature request. Text tags for signers not on the template(s) will be ignored.  **files** or **file_url[]** is required, but not both.. [optional]  # noqa: E501
            file_url ([str]): Use file_url[] to append additional files to the signature request being created from the template. Dropbox Sign will download the file, then parse it for [text tags](https://app.hellosign.com/api/textTagsWalkthrough), and append to the signature request. Text tags for signers not on the template(s) will be ignored.  **files** or **file_url[]** is required, but not both.. [optional]  # noqa: E501
