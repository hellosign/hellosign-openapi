    ...
    public class UnclaimedDraftApiTests
    {
        ...
        public void UnclaimedDraftCreateEmbeddedTest()
        {
            var requestData = TestHelper.GetJsonContents(
                nameof(UnclaimedDraftCreateEmbeddedRequest)
            );
            var responseData = TestHelper.GetJsonContents(nameof(UnclaimedDraftCreateResponse));
        ...
        {
            var requestData = TestHelper.GetJsonContents(
                nameof(UnclaimedDraftCreateEmbeddedWithTemplateRequest)
            );
            var responseData = TestHelper.GetJsonContents(nameof(UnclaimedDraftCreateResponse));

            var obj = UnclaimedDraftCreateEmbeddedWithTemplateRequest.Init(requestData.ToString());
            obj.Files = new List<Stream>
            {
                new FileStream(
                    TestHelper.RootPath + "/pdf-sample.pdf",
        ...
        public void UnclaimedDraftEditAndResendTest()
        {
            ...
            var requestData = TestHelper.GetJsonContents(
                nameof(UnclaimedDraftEditAndResendRequest)
            );
            var responseData = TestHelper.GetJsonContents(nameof(UnclaimedDraftCreateResponse));
