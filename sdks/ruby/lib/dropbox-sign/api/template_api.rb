...
module Dropbox::Sign
  class TemplateApi
    ...
    # @param template_id [String] _t__TemplateEdit::TEMPLATE_ID
    # @param template_edit_request [TemplateEditRequest] 
    # @param [Hash] opts the optional parameters
    # @return [TemplateEditResponse]
    def template_edit(template_id, template_edit_request, opts = {})
    ...
    # @param template_edit_request [TemplateEditRequest] 
    # @param [Hash] opts the optional parameters
    # @return [Array<(TemplateEditResponse, Integer, Hash)>] TemplateEditResponse data, response status code and response headers
    def template_edit_with_http_info(template_id, template_edit_request, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: TemplateApi.template_edit ...'
      end
      # verify the required parameter 'template_id' is set
      if @api_client.config.client_side_validation && template_id.nil?
        fail ArgumentError, "Missing the required parameter 'template_id' when calling TemplateApi.template_edit"
      end
      # verify the required parameter 'template_edit_request' is set
      if @api_client.config.client_side_validation && template_edit_request.nil?
        fail ArgumentError, "Missing the required parameter 'template_edit_request' when calling TemplateApi.template_edit"
      end
      # resource path
      local_var_path = '/template/edit/{template_id}'.sub('{' + 'template_id' + '}', CGI.escape(template_id.to_s))
      ...
      result = @api_client.generate_form_data(
        ...
        Dropbox::Sign::TemplateEditRequest.openapi_types
      )
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'TemplateEditResponse'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['api_key', 'oauth2']

      new_options = opts.merge(
        :operation => :"TemplateApi.template_edit",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: TemplateApi#template_edit\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
