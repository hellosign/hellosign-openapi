...
module Dropbox::Sign
  class SubFormFieldGroup
    ...
    # @return [SubFormFieldGroup]
    def self.init(data)
      ApiClient.default.convert_to_type(
        data,
        "SubFormFieldGroup"
      ) || SubFormFieldGroup.new
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Dropbox::Sign::SubFormFieldGroup` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.merged_attributes.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Dropbox::Sign::SubFormFieldGroup`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] group_label Value to be assigned
    def group_label=(group_label)
      if group_label.nil?
        fail ArgumentError, 'group_label cannot be nil'
      end
    ...
    def requirement=(requirement)
      if requirement.nil?
        fail ArgumentError, 'requirement cannot be nil'
      end
